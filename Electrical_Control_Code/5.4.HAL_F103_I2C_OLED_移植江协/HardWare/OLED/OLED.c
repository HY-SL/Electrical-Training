...
/**
  ...
  * 返 回 值：无
  */
void OLED_WriteData(unsigned char *data, uint16_t length)
{
    HAL_I2C_Mem_Write(&hi2c1, OLED_ADD, 0x40, I2C_MEMADD_SIZE_8BIT, data, length, 100);
}
/**
  * 函    数：OLED写命令
  ...
  * 返 回 值：无
  */
void OLED_WriteCommand(unsigned char i2c_cmd)
{
    uint8_t *cmd;
    cmd = &i2c_cmd;
...
//        OLED_WriteData(OLED_DisplayBuf[j]);
//	}
//}
void OLED_Update(void)
{
